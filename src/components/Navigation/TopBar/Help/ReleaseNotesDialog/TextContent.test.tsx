...
vi.mock('src/api/services/ReleaseNotesService', () => {
  ...
  test('IMG src in TextContent gets token inserted on rendering', async () => {
    ...
    const textWithImg = "<p>hei<br><br><br></p><img alt=\"" + ALT_TEXT + "\" src=\"" + IMG_SRC +  "\"/>"
    
    render(<TextContent text={textWithImg} />, { wrapper: Wrappers });

    await waitFor(() => expect(screen.getByAltText(ALT_TEXT)).toHaveAttribute("src", `${IMG_SRC}?${FAKE_TOKEN}`), { timeout: 200 })
