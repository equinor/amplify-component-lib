...
While a tutorial is showing, pointer events are disabled on the content, so hover, focus, cursor and text selection effects are gone along with clicking. Mouse, touch and keyboard events are blocked as well, to also cover elements rendered in portals. Tabbing is left alone so keyboard users can still reach the popover.

If some elements need to stay interactive, pass their CSS selectors to `interactiveElementSelectors`. To keep the page scrollable, set `allowScrolling`.

```typescript dark
<TutorialHighlightingProvider
  contentRef={contentRef}
  interactiveElementSelectors={['#my-button', '.my-menu']}
  allowScrolling
>
  ...
</TutorialHighlightingProvider>
...
Steps of a tutorial marked as interactive in SAM always keep their highlighted element clickable, no configuration needed.
